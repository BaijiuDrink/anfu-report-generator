...
DEFAULT_VULN_LIBRARY = os.path.join(os.path.dirname(__file__), 'vuln_library', 'default_vulns.json')
...
class VulnManager:
    ...
    def search(self, keyword):
        ...
        for v in self.data.get('vulnerabilities', []):
            if (keyword_lower in v.get('name', '').lower() or
                    keyword_lower in v.get('category', '').lower() or
                    keyword_lower in v.get('id', '').lower()):
                results.append(v)
        return results
    ...
    def update(self, vuln_id, updates):
        ...
        allowed_fields = [
            'name', 'risk_level', 'category', 'description',
            'verify_steps', 'verify_result',
            'fix_suggestion', 'fix_priority', 'fix_verify'
        ]
...
def create_finding(vuln_manager, vuln_id=None, url='',
                   custom_name=None, custom_description=None, custom_risk_level=None,
                   custom_fix_suggestion=None,
                   verify_steps=None, verify_result=None,
                   fix_priority=None, fix_verify=None,
                   network_zone='互联网', host_ip=''):
    finding = {
        'vuln_id': vuln_id,
        'name': custom_name,
        'url': url,
...
def batch_create_findings(vuln_manager, vuln_id, hosts, network_zone='内网',
                           custom_name=None, custom_risk_level=None,
                           verify_steps=None, verify_result=None):
    template = vuln_manager.get_by_id(vuln_id)
    if not template:
