...
def _select_vuln_interactive(vuln_manager):
    ...
    for i, v in enumerate(vulns):
        print(f'{i + 1:<4}{v.get("id", ""):<14}{v.get("name", ""):<24}{v.get("category", ""):<12}{v.get("risk_level", "")}')
    print('-' * 70)
        ...
        if choice.isdigit():
            ...
            if results:
                ...
                for i, v in enumerate(results):
                    print(f'  [{i + 1}] {v.get("id")} - {v.get("name")} ({v.get("risk_level")})')
...
def _input_finding_interactive(vuln_manager):
    ...
    if vuln_template:
        ...
        finding['name'] = input(f'漏洞名称 [{vuln_template["name"]}]: ').strip() or vuln_template['name']
        finding['risk_level'] = input(f'风险等级 [{vuln_template["risk_level"]}]: ').strip() or vuln_template['risk_level']
        ...
        if use_default_fix in ('n', 'no'):
            ...
            while True:
                ...
                lines.append(line)
            if lines:
                finding['fix_suggestion'] = '\n'.join([finding['fix_suggestion']] + lines)
            else:
        ...
        if not finding['name']:
            print('[错误] 漏洞名称不能为空')
            return None
        finding['risk_level'] = input(f'风险等级 ({"/".join(RISK_LEVELS)}): ').strip() or '中危'
...
def run_library_list(args):
    ...
    print(f'共 {len(vulns)} 条漏洞记录:\n')
    for v in vulns:
        print(f'  [{v.get("id")}] {v.get("name")} | {v.get("category")} | {v.get("risk_level")}')
    print('')
...
def run_library_add(args):
    ...
    vuln['risk_level'] = input(f'风险等级 ({"/".join(RISK_LEVELS)}): ').strip() or '中危'
...
def run_library_delete(args):
    ...
    if not existing:
        print(f'[错误] 漏洞ID [{vuln_id}] 不存在')
        return
    confirm = input(f'确认删除 [{vuln_id}] {existing.get("name")}? (y/N): ').strip().lower()
    if confirm == 'y':
        vm.delete(vuln_id)
        print(f'[已删除] {vuln_id}')
    else:
...
使用示例:
  ...
  python report_generator.py --init-config
    ...
    parser.add_argument('-c', '--config', help='配置文件路径 (JSON/YAML)，从配置文件生成报告')
    parser.add_argument('-o', '--output', help='输出文件路径 (默认: output/项目名称_渗透测试报告.docx)')
    parser.add_argument('-l', '--library', default=DEFAULT_LIBRARY, help='漏洞库文件路径')
    ...
    sub.add_argument('--delete', dest='delete_id', metavar='ID', help='删除指定ID的漏洞')
