...
from vuln_manager import VulnManager, RISK_LEVELS, FIX_PRIORITIES, NETWORK_ZONES, batch_create_findings
...
class PentestReportApp:
    ...
    def _setup_styles(self):
        ...
        style.configure('Title.TLabel', font=('微软雅黑', 16, 'bold'), foreground='#2F5496')
        style.configure('Section.TLabel', font=('微软雅黑', 11, 'bold'), foreground='#333333')
    ...
    def _build_top_bar(self):
        ...
        ttk.Label(top, text='渗透测试报告生成工具', style='Title.TLabel').pack(side=tk.LEFT, padx=(0, 20))

        ttk.Label(top, text='项目名称:').pack(side=tk.LEFT, padx=(10, 4))
        self.project_var = tk.StringVar(value='XXX公司渗透测试')
        ttk.Entry(top, textvariable=self.project_var, width=30, font=('微软雅黑', 10)).pack(side=tk.LEFT, padx=(0, 10))

        ttk.Button(top, text='新建项目', command=self._clear_findings).pack(side=tk.LEFT, padx=4)
        ttk.Button(top, text='加载配置', command=self._load_config).pack(side=tk.LEFT, padx=4)
        ttk.Button(top, text='保存配置', command=self._save_config).pack(side=tk.LEFT, padx=4)
        ttk.Button(top, text='批量录入', command=self._batch_add_dialog).pack(side=tk.LEFT, padx=4)
        ttk.Button(top, text='生成报告', command=self._generate_report).pack(side=tk.LEFT, padx=4)
    ...
    def _build_left_panel_in(self, parent):
        ...
        ttk.Label(left, text='漏洞库', style='Section.TLabel').grid(row=0, column=0, sticky='w', pady=(0, 2))

        search_frame = ttk.Frame(left)
        search_frame.grid(row=1, column=0, sticky='ew', pady=(0, 4))
        self.search_var = tk.StringVar()
        ttk.Entry(search_frame, textvariable=self.search_var, width=22, font=('微软雅黑', 9)).pack(side=tk.LEFT, fill=tk.X, expand=True)
        ttk.Button(search_frame, text='搜索', command=self._search_library, width=5).pack(side=tk.LEFT, padx=(2, 0))
        self.search_var.trace_add('write', lambda *a: self._search_library())

        columns = ('name', 'risk', 'category')
        self.lib_tree = ttk.Treeview(left, columns=columns, show='headings',
        ...
        lib_scroll = ttk.Scrollbar(left, orient=tk.VERTICAL, command=self.lib_tree.yview)
        ...
        ttk.Button(btn_frame, text='添加到项目', command=self._add_from_library).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text='管理漏洞库', command=self._open_library_manager).pack(side=tk.LEFT, padx=2)

        left.grid_rowconfigure(4, weight=1)
        self.lib_preview_text = tk.Text(left, height=8, font=('微软雅黑', 9), wrap=tk.WORD, state='disabled')
        self.lib_preview_text.grid(row=4, column=0, columnspan=2, sticky='nsew', pady=(4, 0))
    ...
    def _build_right_panel_in(self):
        ...
        ttk.Label(right, text='已录入漏洞列表', style='Section.TLabel').grid(row=0, column=0, sticky='w')

        f_btn_frame = ttk.Frame(right)
        f_btn_frame.grid(row=0, column=0, sticky='e')
        ttk.Button(f_btn_frame, text='↑上移', command=lambda: self._move_finding(-1), width=5).pack(side=tk.LEFT, padx=1)
        ttk.Button(f_btn_frame, text='↓下移', command=lambda: self._move_finding(1), width=5).pack(side=tk.LEFT, padx=1)
        ttk.Button(f_btn_frame, text='编辑', command=self._edit_finding, width=5).pack(side=tk.LEFT, padx=1)
        ttk.Button(f_btn_frame, text='复制', command=self._copy_finding, width=5).pack(side=tk.LEFT, padx=1)
        ttk.Button(f_btn_frame, text='×删除', command=self._delete_finding, width=5).pack(side=tk.LEFT, padx=1)

        self.right_pane = tk.PanedWindow(right, orient=tk.VERTICAL, sashwidth=5, sashrelief=tk.RAISED)
    ...
    def _build_findings_list(self):
        ...
        self.findings_tree = ttk.Treeview(list_frame, columns=f_columns, show='headings',
                                           height=5, style='Finding.Treeview')
        ...
        f_scroll = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.findings_tree.yview)
    ...
    def _build_editor_panel_in(self):
        editor_frame = ttk.LabelFrame(self.right_pane, text='漏洞编辑区', padding=(8, 4, 8, 8))
        ...
        row = 0
        ttk.Label(editor_frame, text='漏洞名称:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='w', pady=1)
        self.editor_name = tk.StringVar()
        ttk.Entry(editor_frame, textvariable=self.editor_name, font=('微软雅黑', 9)).grid(row=row, column=1, sticky='ew', pady=1, padx=(4, 0))

        row = 1
        ttk.Label(editor_frame, text='漏洞地址:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='nw', pady=1)
        self.editor_url = tk.Text(editor_frame, height=3, font=('微软雅黑', 9), wrap=tk.WORD)
        self.editor_url.grid(row=row, column=1, sticky='nsew', pady=1, padx=(4, 0))

        row = 2
        ttk.Label(editor_frame, text='网络区域:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='w', pady=1)
        ...
        for z in NETWORK_ZONES:
            ttk.Radiobutton(zone_frame, text=z, value=z, variable=self.editor_zone).pack(side=tk.LEFT, padx=2)

        row = 3
        ttk.Label(editor_frame, text='风险等级:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='w', pady=1)
        ...
        for r in RISK_LEVELS_SHORT:
            ttk.Radiobutton(risk_frame, text=r, value=r, variable=self.editor_risk).pack(side=tk.LEFT, padx=2)

        row = 4
        ttk.Label(editor_frame, text='修复优先级:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='w', pady=1)
        ...
        for p in FIX_PRIORITIES:
            ttk.Radiobutton(pri_frame, text=p, value=p, variable=self.editor_priority).pack(side=tk.LEFT, padx=2)

        row = 5
        ttk.Label(editor_frame, text='漏洞描述:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='nw', pady=1)
        self.editor_description = tk.Text(editor_frame, height=3, font=('微软雅黑', 9), wrap=tk.WORD)
        self.editor_description.grid(row=row, column=1, sticky='nsew', pady=1, padx=(4, 0))

        row = 6
        ttk.Label(editor_frame, text='漏洞验证:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='nw', pady=1)
        self.editor_verify_steps = tk.Text(editor_frame, height=8, font=('微软雅黑', 9), wrap=tk.WORD)
        self.editor_verify_steps.grid(row=row, column=1, sticky='nsew', pady=1, padx=(4, 0))
        self._bind_paste_handler(self.editor_verify_steps)

        row = 7
        ttk.Label(editor_frame, text='验证结果:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='nw', pady=1)
        self.editor_verify_result = tk.Text(editor_frame, height=2, font=('微软雅黑', 9), wrap=tk.WORD)
        self.editor_verify_result.grid(row=row, column=1, sticky='nsew', pady=1, padx=(4, 0))

        row = 8
        ttk.Label(editor_frame, text='修复建议:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='nw', pady=1)
        self.editor_fix_suggestion = tk.Text(editor_frame, height=3, font=('微软雅黑', 9), wrap=tk.WORD)
        self.editor_fix_suggestion.grid(row=row, column=1, sticky='nsew', pady=1, padx=(4, 0))

        row = 9
        ttk.Label(editor_frame, text='整改验证方法:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='nw', pady=1)
        self.editor_fix_verify = tk.Text(editor_frame, height=2, font=('微软雅黑', 9), wrap=tk.WORD)
        self.editor_fix_verify.grid(row=row, column=1, sticky='nsew', pady=1, padx=(4, 0))

        row = 10
        btn_row = ttk.Frame(editor_frame)
        btn_row.grid(row=row, column=0, columnspan=2, sticky='e', pady=(4, 0))
        ttk.Button(btn_row, text='✓ 保存漏洞', command=self._save_current_finding).pack(side=tk.RIGHT, padx=2)
        ttk.Button(btn_row, text='清空表单', command=self._clear_editor).pack(side=tk.RIGHT, padx=2)
    ...
    def _show_editor(self, finding_template=None):
        ...
        if finding_template:
            ...
            self.editor_description.insert('1.0', finding_template.get('description', ''))
            self.editor_risk.set(finding_template.get('risk_level', '中危'))
            self.editor_verify_steps.insert('1.0', finding_template.get('verify_steps', ''))
            self.editor_verify_result.insert('1.0', finding_template.get('verify_result', ''))
            self.editor_fix_suggestion.insert('1.0', finding_template.get('fix_suggestion', ''))
    ...
    def _clear_editor(self):
        ...
        for w in [self.editor_description, self.editor_verify_steps,
                   self.editor_verify_result, self.editor_fix_suggestion,
    ...
    def _insert_with_images(self, text_widget, content):
        import re
        pattern = re.compile(r'\[截图:\s*(screenshots/[^\]]+\.(?:png|jpg|jpeg|gif|bmp))\]', re.IGNORECASE)
        last_end = 0
    ...
    def _move_finding(self, direction):
        ...
        if 0 <= new_idx < len(self.findings):
            self.findings[idx], self.findings[new_idx] = self.findings[new_idx], self.findings[idx]
            self._refresh_findings_list()
    ...
    def _batch_add_dialog(self):
        ...
        ttk.Label(frm, text='漏洞库ID:', font=('微软雅黑', 9, 'bold')).grid(row=0, column=0, sticky='w', pady=2)
        id_var = tk.StringVar(value=initial_id)
        ttk.Entry(frm, textvariable=id_var, font=('微软雅黑', 9), width=25).grid(row=0, column=1, sticky='w', padx=(8, 0), pady=2)

        ttk.Label(frm, text='网络区域:', font=('微软雅黑', 9, 'bold')).grid(row=1, column=0, sticky='w', pady=2)
        zone_var = tk.StringVar(value='内网')
        zf = ttk.Frame(frm)
        zf.grid(row=1, column=1, sticky='w', padx=(8, 0), pady=2)
        for z in NETWORK_ZONES:
            ttk.Radiobutton(zf, text=z, value=z, variable=zone_var).pack(side=tk.LEFT, padx=4)

        ttk.Label(frm, text='主机列表\n(一行一个IP/URL):', font=('微软雅黑', 9, 'bold')).grid(row=2, column=0, sticky='nw', pady=2)
        hosts_text = tk.Text(frm, height=8, font=('微软雅黑', 9), wrap=tk.WORD)
        hosts_text.grid(row=2, column=1, sticky='ew', padx=(8, 0), pady=2)

        ttk.Label(frm, text='自定义名称\n(留空则用漏洞库名):', font=('微软雅黑', 9, 'bold')).grid(row=3, column=0, sticky='nw', pady=2)
        name_var = tk.StringVar()
        ttk.Entry(frm, textvariable=name_var, font=('微软雅黑', 9), width=25).grid(row=3, column=1, sticky='ew', padx=(8, 0), pady=2)
        ...
        def _do_batch():
            ...
            try:
                ...
                self._refresh_findings_list()
                messagebox.showinfo('完成', f'已批量录入 {len(batch)} 条漏洞记录', parent=dialog)
                dialog.destroy()
            except ValueError as e:
                messagebox.showerror('错误', str(e), parent=dialog)

        ttk.Button(frm, text='执行批量录入', command=_do_batch).grid(row=4, column=0, columnspan=2, pady=(10, 0))
        ttk.Label(frm, text='提示：每行一个IP地址或URL，漏洞描述/修复建议自动从漏洞库读取',
                  font=('微软雅黑', 8), foreground='gray').grid(row=5, column=0, columnspan=2, pady=(4, 0))
    ...
    def _emf_to_image(self, emf_data):
        ...
        try:
            ...
            try:
                ...
                hbmp = gdi32.CreateDIBSection(mdc, ctypes.byref(bmi), 0, ctypes.byref(pBits), None, 0)
                if not hbmp:
                    return None
                try:
                    ...
                    if hemf:
                        ...
                        gdi32.PlayEnhMetaFile(mdc, hemf, ctypes.byref(wintypes.RECT(0, 0, w, h)))
        ...
        def _img_src_to_image(src):
            try:
                if src.startswith('data:image'):
                    ...
                    if b64_match:
                        return Image.open(io.BytesIO(base64.b64decode(b64_match.group(1))))
                ...
                while True:
                    ...
                    try:
                        ...
                        if dib_size <= 0 or dib_size > len(data):
                            continue
                        bmp_header = struct.pack('<2sIHHI', b'BM', 14 + len(data), 0, 0, 14 + dib_size)
...
class LibraryManagerDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        self.tree = ttk.Treeview(tree_frame, columns=columns, show='headings',
                                  height=15, style='Library.Treeview')
        ...
        self.detail_text = tk.Text(detail_frame, width=35, font=('微软雅黑', 9), wrap=tk.WORD,
                                    state='disabled')
    ...
    def _refresh_list(self):
        ...
        for v in self.vuln_manager.list_all():
            self.tree.insert('', tk.END, values=(
                v.get('id', ''), v.get('name', ''), v.get('risk_level', ''), v.get('category', '')
            ))
...
class VulnEditDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        for label_text, attr in labels:
            ttk.Label(frame, text=label_text, font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='w', pady=3)
            var = tk.StringVar()
            ttk.Entry(frame, textvariable=var, font=('微软雅黑', 9), width=40).grid(row=row, column=1, sticky='ew', pady=3, padx=(8, 0))
            self.widgets[attr] = var
            row += 1

        ttk.Label(frame, text='风险等级:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='w', pady=3)
        ...
        for r in RISK_LEVELS_SHORT:
            ttk.Radiobutton(risk_frame, text=r, value=r, variable=self.widgets['risk_var']).pack(side=tk.LEFT, padx=2)
        row += 1

        ttk.Label(frame, text='修复优先级:', font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='w', pady=3)
        ...
        for p in FIX_PRIORITIES:
            ttk.Radiobutton(pri_frame, text=p, value=p, variable=self.widgets['priority_var']).pack(side=tk.LEFT, padx=2)
        row += 1
        ...
        for label_text, attr, height in text_fields:
            ttk.Label(frame, text=label_text, font=('微软雅黑', 9, 'bold')).grid(row=row, column=0, sticky='nw', pady=3)
            txt = tk.Text(frame, height=height, font=('微软雅黑', 9), wrap=tk.WORD, width=40)
    ...
    def _load_existing(self):
        ...
        for field, attr in [('description', 'description_text'), ('verify_steps', 'verify_steps_text'),
                             ('verify_result', 'verify_result_text'), ('impact_scope', 'impact_scope_text'),
                             ('fix_suggestion', 'fix_suggestion_text'), ('fix_verify', 'fix_verify_text')]:
